...
begin
  ...
  butMes:= MessageDlg('Вы действительно хотите удалить заметку №'+a+'?', mtCustom, [mbYes, mbCancel], 0);
  if butMes = mrYes
  then
    begin
      ADOQuery1.Close;
...
begin
  ...
  if SaveDialog1.Execute then
    begin
      ...
      Rewrite(F);
      WriteLn(F, '№ '+ADOTable1.Fields.Fields[0].AsString + ' | Дата: '+ADOTable1.Fields.Fields[1].AsString);
      WriteLn(F, '');
      WriteLn(F, '    Cодержание:');
      WriteLn(F, '');
      WriteLn(F, ADOTable1.Fields.Fields[2].AsString);
      CloseFile(F);
    end;
end;
...
begin
  ShowMessage('Тестовое задание для разработчиков "Заметки"'+ #13#10 + #13#10 +'Разработала: Суровцева Виолина Михайловна');
end;
